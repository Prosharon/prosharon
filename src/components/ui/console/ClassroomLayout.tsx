...
import { TbFile, TbFileText, TbFileUpload, TbFolderPlus, TbTrash, TbVideoPlus, TbVideo, TbFolder } from "react-icons/tb";
import { functions } from "@/services/firebase";
import CreateFolderModal from "../modals/CreateFolderModal";
import { httpsCallable } from "firebase/functions";

const ClassroomLayout = ({classroomId}: GeneralDocument) => {
	const [isCreateFolderOpen, setIsCreateFolderOpen] = useState<boolean>(false);
	const makeFolder = async () => {
		const addSectionIfInstructor = httpsCallable(functions, 'addSectionIfInstructor');
		try {
		const result = await addSectionIfInstructor({
			classroomId: classroomId,
			sectionName: (document.getElementById('folder-name') as HTMLInputElement).value,
	...
	return (
		<ConsoleCard
			...
			notHoverable={true}
		>
			<CreateFolderModal isOpen={isCreateFolderOpen} onClose={() => setIsCreateFolderOpen(false)} makeFolder={makeFolder} />
			<div className="flex flex-wrap justify-between items-center mb-6 space-y-4 md:space-y-0">
				...
				<div className="flex flex-wrap justify-end space-y-3 space-x-0 md:space-y-0 md:space-x-3 w-full md:w-auto font-bold text-sm">
					...
					<button onClick={() => setIsCreateFolderOpen(true)} className="bg-azure dark:bg-azure text-white px-4 py-2 rounded-xl flex items-center space-x-2 hover:bg-opacity-90 transition w-full md:w-auto">
			...
			<div className="space-y-6 text-gray-800 dark:text-gray-300">
				...
				{/* Folder - Geometric Representations */}
				<div className="p-4 bg-gray-100 dark:bg-gray-800 rounded-lg shadow-sm">
					<div className="flex justify-between items-center mb-4">
						<div className="flex items-center space-x-3">
							<TbFolder
